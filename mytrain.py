...
def get_img_batch(files_list,
    ...
    for i in range(batch_size):
        img_cover_path = random.choice(files_list)#.choice()返回一个列表，元组或字符串的随机项。
        try:
        ...
        batch_cover.append(img_cover)#append()用于在列表末尾添加新的对象,无返回值，但是会修改原来的列表。extend()方法只接受一个列表作为参数，并将该参数的每个元素都添加到原有的列表中。
